...
export function normalizeCaptionGroup(value: unknown, index: number, durationMs: number, fallbackStyleId: string): CaptionGroup | null {
  ...
  const group: CaptionGroup = {
    ...
    reviewNote: stringValue(entry.reviewNote).trim() || undefined,
    locked: entry.locked === true || undefined,
    contentTag: entry.contentTag === "chapter" || entry.contentTag === "highlight" ? entry.contentTag : undefined,
    startMs,
    endMs,

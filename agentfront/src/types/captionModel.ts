import type { AssStyle, CaptionEffect, CaptionOverrides, CaptionUnit, Subtitle, WordTimestamp } from "./subtitleTypes";

export type { CaptionEffect, CaptionOverrides, CaptionUnit, CaptionEffectType, WordTimestamp } from "./subtitleTypes";
...
export const subtitleToCaptionGroup = (subtitle: Subtitle, styleId = subtitle.style || "Default"): CaptionGroup => ({
  id: subtitle.id,
  text: subtitle.text,
...
export const splitGroupByCharacters = (group: CaptionGroup, words?: WordTimestamp[]): CaptionGroup => {
  if (words?.length) {
...
export const splitGroupByWords = (group: CaptionGroup, words?: WordTimestamp[]): CaptionGroup => {
  const source = words?.length ? words : group.text.trim().split(/\s+/).filter(Boolean).map((word, index, all) => ({
    word,
    start: (group.startMs + (group.endMs - group.startMs) * index / all.length) / 1000,
    end: (group.startMs + (group.endMs - group.startMs) * (index + 1) / all.length) / 1000,
    timingSource: "estimated" as const,
  }));
  return { ...group, units: source.map((item, index) => ({
    id: `${group.id}-unit-${index + 1}`,
    text: item.word,
...
export const splitGroupByPunctuation = (group: CaptionGroup): CaptionGroup => {
  const matches = [...group.text.matchAll(/[^，。！？；,.!?;]+[，。！？；,.!?;]?/gu)].map(match => match[0]);
  if (matches.length <= 1) return splitGroupByCharacters(group);
  const duration = Math.max(0, group.endMs - group.startMs);
  let offset = 0;
  const units = matches.map((text, index) => {
    const startMs = group.startMs + Math.floor(duration * offset / group.text.length);
    offset += text.length;
    const endMs = group.startMs + Math.floor(duration * offset / group.text.length);
    return { id: `${group.id}-unit-${index + 1}`, text, startMs, endMs, overrides: {}, timingSource: "estimated" as const, order: index };
  });
  return { ...group, units };
};

export const mergeCaptionGroups = (groups: CaptionGroup[]): CaptionGroup | null => {
  if (groups.length === 0) return null;
  const ordered = [...groups].sort((a, b) => a.startMs - b.startMs);
  const first = ordered[0];
  const mergedUnits = ordered.flatMap(group => group.units).map((unit, index) => ({ ...unit, id: `${first.id}-unit-${index + 1}`, order: index }));
  return {
    ...first,
...
export const mergeOverrides = (style: AssStyle | undefined, overrides: CaptionOverrides): CaptionOverrides => ({
...
export const splitSubtitleAtGraphemeIndex = (
  ...
  if (graphemes.length < 2 || splitIndex <= 0 || splitIndex >= graphemes.length) return null;
  ...
  const left: Subtitle = { ...subtitle, id: `${Date.now()}_${Math.random().toString(16).slice(2)}`, text: leftText, end: midSec, style: styleId, overrides: { ...(subtitle.overrides || {}) }, units: [], effect: undefined };
  const right: Subtitle = { ...subtitle, id: `${Date.now()}_${Math.random().toString(16).slice(2)}`, text: rightText, start: midSec, style: styleId, overrides: { ...(subtitle.overrides || {}) }, units: [], effect: undefined };
  return [left, right];
};
